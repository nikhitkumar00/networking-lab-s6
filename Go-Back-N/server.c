...
int main()
{
    ...
    while (1)
    {
        ...
        int receivedSequenceNumber = buffer[0] - '0';
        printf("Received frame %d: %s\n", receivedSequenceNumber, buffer + 1);
        ...
        {
            // Discard out-of-order frames and send negative acknowledgement (NAK)
            int nak = -1;
            send(clientSocket, &nak, sizeof(int), 0);
        }
    }
